...
DEFAULT_MODEL_FILES = ["config.json", "onnx/model.onnx", "special_tokens_map.json", "tokenizer.json",
...
def get_model() -> str:
    for file in DEFAULT_MODEL_FILES:
        if not os.path.exists(os.path.join("local_model", "all-MiniLM-L6-v2", file)):
            hf_hub_download(repo_id=DEFAULT_REPO, filename=file, local_dir="./local_model/all-MiniLM-L6-v2")
    return os.path.join("local_model", "all-MiniLM-L6-v2")


def test_download(get_model):
    print(get_model)
    ef = OnnxRuntimeEmbeddings(model_path=get_model, preferred_providers=["CPUExecutionProvider"])
